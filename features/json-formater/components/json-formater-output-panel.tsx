...
type JsonFormaterOutputPanelProps = {
  jsonOutput: string | null
  onCopy: () => void
  onClear: () => void
}

export function JsonFormaterOutputPanel({
  jsonOutput,
  onCopy,
  onClear,
}: JsonFormaterOutputPanelProps) {
  if (!jsonOutput) {
    return null
  }
  ...
  return (
    <section className="rounded-[2rem] bg-card p-5 shadow-lg ring-1 ring-foreground/5 sm:p-6 lg:col-span-12">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="font-heading text-xl font-semibold">Parsed JSON</h2>
          <p className="text-sm leading-6 text-muted-foreground">
            Exact value after parse, pretty-printed with 2-space indent.
          </p>
        </div>
